        ...
        doSearch: function (classId) {
            ...
            $.post(Report.queryTestInfoListUrl, condition, function (data) {
                if (data.data.length > 0) {
                   ...
                   /* var testInfoList = [];
                    for (let i = 0; i < 5; i++) {
                        $.each(data.data, function (index, testInfo) {
                            testInfoList.push(testInfo);
                        })
                    }
                    _this.diagnosis.testInfoList = testInfoList;*/
                    _this.diagnosis.testInfoList = data.data;
                }else {
                    _this.diagnosis.testInfoList = [];
                }
                layer.close(index);
            });
        },
...
var Report = window.Report = {
    ...
    initVue: function () {
        VueConfig.data.diagnosis = this.diagnosis;
    ...
    initDiagnosis: function () {
        ...
        $("#diagnosis").addClass("activeLi");
        _this.diagnosis.testInfoList = _this.testInfoList;
    },
    initEvent: function (){
        var _this = this;
